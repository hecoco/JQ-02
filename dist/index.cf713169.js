window.$ = window.jQuery = function(selectorOrArray) {
    ...
    return {
        ...
        each (fn) {
            for(let i = 0; i < elements.length; i++)fn.call(null, elements[i], i) //传参过去，直接可以调用
            ;
            return this;
        },
        parent () {
        ...
        children () {
            const array = [];
            this.each((node)=>{
                if (array.indexOf(node.children) === -1) array.push(...node.children); //展开操作符  把node.children里面的数组拿出来
            });
            return jQuery(array);
        },
        siblings () {
        ...
        next () {
            ...
            this.each((node)=>{
                x = node.nextSibling;
                if (x && x.nodeType === 3) x = x.nextSibling;
            });
            return x;
        },
        prev () {
            ...
            this.each((node)=>{
                ...
                if (x && x.nodeType === 3) x = x.previousSibling;
            });
            return x;
        },
        //在节点前面增加兄弟节点
...
//# sourceMappingURL=index.cf713169.js.map
